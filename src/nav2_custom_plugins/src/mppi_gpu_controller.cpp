  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  DECLARE_GET_PARAM(node_ptr, plugin_name_, "use_geometric_heading_for_alignment",
                    use_geometric_heading_for_alignment_);
...
void MPPIGPUController::setPlan(const nav_msgs::msg::Path & path)
{
  ...
  if (!raw_global_plan_.poses.empty() && !path.poses.empty()) {
    double old_gx = raw_global_plan_.poses.back().pose.position.x;
    double old_gy = raw_global_plan_.poses.back().pose.position.y;
  ...
  nav2_core::GoalChecker *)
{
  ...
  // 全局规划器给出的 Path 通常在 map 下，而本控制器收到的 pose 和局部
  // costmap 通常在 odom 下。任何距离、最近点和碰撞计算前必须先统一坐标系。
  global_plan_ = raw_global_plan_;
  if (!raw_global_plan_.poses.empty()) {
    const std::string target_frame = pose.header.frame_id.empty() ?
      costmap_ros_->getGlobalFrameID() : pose.header.frame_id;
    std::string source_frame = raw_global_plan_.header.frame_id;
    if (source_frame.empty()) {
      source_frame = raw_global_plan_.poses.front().header.frame_id;
    }

    if (source_frame.empty() || target_frame.empty()) {
      RCLCPP_ERROR_THROTTLE(
        node_.lock()->get_logger(), *node_.lock()->get_clock(), 2000,
        "无法转换全局路径：source_frame='%s', target_frame='%s'，本周期输出零速度",
    ...
    if (source_frame != target_frame) {
      try {
        ...
        global_plan_.header.stamp = pose.header.stamp;
        global_plan_.poses.clear();
        global_plan_.poses.reserve(raw_global_plan_.poses.size());
        for (const auto & raw_pose : raw_global_plan_.poses) {
          geometry_msgs::msg::PoseStamped source_pose = raw_pose;
          ...
          global_plan_.poses.push_back(std::move(transformed_pose));
        }
      } catch (const tf2::TransformException & ex) {
        RCLCPP_ERROR_THROTTLE(
          node_.lock()->get_logger(), *node_.lock()->get_clock(), 2000,
          "全局路径从 %s 转换到 %s 失败：%s；本周期输出零速度",
  ...
  // ══════════════════════════════════════════
  {
    ...
    double h_err = 0.0;
    // G1 是非完整约束平台（这里只允许前进 + 转向）。NavFn 等二维规划器
    // 不保证每个 Path pose 的 orientation 可用于运动朝向判断；若它保持为 0，
    // 当前方几何路径实际位于机器人身后时，控制器会错误留在 NORMAL，且又因
    // min_v=0/max_vy=0 无法后退或侧移，最终陷入接近零速。G1 因此使用机器人
    // 指向主前瞻点的几何角度；默认分支保留上游插件原有的 Path pose 朝向语义。
    if (!global_plan_.poses.empty() && lh_idx < static_cast<int>(global_plan_.poses.size())) {
      if (use_geometric_heading_for_alignment_) {
        h_err = std::atan2(target_y - current_y, target_x - current_x) - current_theta;
      } else {
        const double rec_yaw = tf2::getYaw(global_plan_.poses[lh_idx].pose.orientation);
